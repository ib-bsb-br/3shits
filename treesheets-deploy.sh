...
#              Includes robust build diagnostics and a fallback build path with
...
report_lobster_patch_state() {
    local lobster_dir
    lobster_dir=$(find "$SRC_DIR/$CMAKE_BUILD_DIR/_deps" -maxdepth 2 -type d -name lobster-src | head -n 1 || true)

    if [[ -z "$lobster_dir" ]]; then
        echo "Warning: Lobster source tree not found. If ENABLE_LOBSTER=ON, inspect CMake configure output." >&2
        return 1
    fi
...
if ! sudo apt-get -o Acquire::Retries=3 update; then
    echo "Warning: APT update returned errors (possibly third-party repo key issues). Continuing..." >&2
fi

echo "Installing core build tools and dependencies..."
ensure_pkg_installed "build-essential"
ensure_tool_installed "cmake"
ensure_tool_installed "git"
ensure_tool_installed "gettext"

# C++ stacktrace/debugging deps (cpptrace/fault)
ensure_pkg_installed "libunwind-dev"
ensure_pkg_installed "libdw-dev"

# GUI/OpenGL rendering deps + runtime popup tool
...
echo "============================================================"
echo "Section 2: Native C++ Compilation"
echo "============================================================"

cd "$WORK_DIR"
if [[ -d "$SRC_DIR" ]]; then
    echo "--> Source directory $SRC_DIR already exists. Pulling latest changes..."
    (cd "$SRC_DIR" && git pull || echo "Warning: Git pull failed. Continuing with existing source.")
else
...
configure_cmake "ON"
report_lobster_patch_state || true
...
echo "--> Build artifacts/logs: $SRC_DIR/$CMAKE_BUILD_DIR"
...
read -r -p "WARNING: Configure TreeSheets kiosk auto-start on boot (systemd + Xorg on :0)? (yes/NO): " setup_kiosk
