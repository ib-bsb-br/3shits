...
# packaging, optional install, and optional systemd kiosk setup.
#
# Design notes:
# - Primary fix for GCC10/Lobster charconv lives in CMakeLists.txt.
# - Script keeps a verified patch fallback in case source tree predates that fix.
...
patch_lobster_charconv_if_needed() {
    ...
    pre_count=$(rg -l "__cpp_lib_to_chars" "$lobster_dir" | wc -l || true)
    ...
    post_count=$(rg -l "__cpp_lib_to_chars" "$lobster_dir" | wc -l || true)
...
if [ "$build_rc" -ne 0 ]; then
    ...
    echo "--> Retrying with ENABLE_LOBSTER=OFF..."
    python - <<'PY'
import shutil
shutil.rmtree("_build", ignore_errors=True)
PY
...
if [ -z "$DEB_PACKAGE" ]; then
    echo "Error: Could not find generated .deb package." >&2
    exit 1
fi
...
EOF_SERVICE"
    ...
    if [[ "$start_kiosk" == "yes" ]]; then
        ...
        sudo systemctl start treesheets-kiosk.service || echo "Warning: Failed to start kiosk service."
    fi
fi
