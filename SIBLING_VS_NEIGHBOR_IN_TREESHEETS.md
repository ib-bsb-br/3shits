...
## 11) Operational decision procedure (quick classifier)

Use this when classifying any pair `(a,b)`:

1. Check parent/owner identity first.
   - If same parent and same owner grid (and `a != b`) => they are siblings.
2. Check context-specific adjacency separately.
   - If adjacent under chosen context model => they are neighbors.
3. Report independently:
   - `Sibling=true/false`
   - `Neighbor=true/false (context=X)`

Possible outputs:

- `Sibling=true, Neighbor=true`  (co-origin and locally close)
- `Sibling=true, Neighbor=false` (co-origin but not locally close in this context)
- `Sibling=false, Neighbor=true` (different ancestry but locally close)
- `Sibling=false, Neighbor=false` (neither)

This avoids semantic collapse and keeps framework3 contract checks composable.
